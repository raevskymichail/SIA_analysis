...
get_pathways_activation_sample_annotations <- function(pathways_activation_db_path = "data/pathways_activation_scores/EGF_PWs.xlsx",
                                                       ...
                                                       return_ranks = FALSE) {
  pathways_activation_db <- read_excel(pathways_activation_db_path, sheet = "PAL2")
  ...
  sample_cols <- !grepl("Norm_geomean.*|Case_geomean.*", colnames(pathways_activation_db), ignore.case = TRUE)
  ...
  colnames(pathways_activation_db) <- gsub("Norm_", "", colnames(pathways_activation_db))
  colnames(pathways_activation_db) <- gsub("Case_", "", colnames(pathways_activation_db))
  ...
  if (return_dataframe == FALSE) {
    ...
    sample_ann <- as.data.frame(pathways_activation_db)
    rownames(sample_ann) <- pathways_names
    sample_ann$Pathway <- NULL
    sample_ann$Database <- NULL
  }
...
#' and cols are TCGA sample names (case IDs)
#' @param sample_ann a named vector, where names are TCGA sample names (case IDs)
#' @param condition_name a string, specifying expected comparison for `DESeq2`. For ex. "condition_Mutated_vs_Control",
#' where `Mutated` and `Control` are values, presented in \code{sample_ann}
...
#' @export
dif_exp_analysis <- function(exp_matrix, sample_ann, condition_name = NULL, shring_results = FALSE) {
  ...
  if (!is.null(condition_name)) {
    ...
    states <- unique(sample_ann)
    control_state <- states[grepl("[Nn]orm|[Cc]ontrol|GTEx", states)]
    case_state <- states[!states %in% control_state]
    contrast <- c("condition", case_state, control_state)
  ...
  if (shring_results == TRUE) {
    ...
    if (!is.null(condition_name)) {
      res <- lfcShrink(dds, coef = condition_name, type = "apeglm")
    }
...
#' Set selected column as a rownames of a dataframe and removes that column
#'
#' @param df a data frame
#' @param col_name chr, name of column in \code{df}
#' @export
column_to_rownames <- function(df, col_name) {
  df <- as.data.frame(df)
...
#' Obtain expression samples annotation
#'
#' @param sample_ann_table_path path to sample annotations table
#' @param sample_list_path optional. Path to `.txt` file containing a list of TCGA patient codes to subset from the expression matrix.
#' @return a named vector, where values are source of exp sample and names are Sample ID
#'
#' @import readr
#'
#' @export
get_exp_samples_annotations <- function(sample_ann_table_path = "data/exp_data/exp_samples_annotation.csv",
                                        sample_list_path = NULL) {
  sample_ann_table <- read.csv(sample_ann_table_path)
  sample_ann <- sample_ann_table$Source
  names(sample_ann) <- sample_ann_table$Sample_ID

  if (!is.null(sample_list_path)) {
    sample_list <- read_delim(sample_list_path,
        "\t", col_names = FALSE, escape_double = FALSE, trim_ws = TRUE)
    common_samples <- intersect(names(sample_ann), sample_list$X1)
    sample_ann <- sample_ann[common_samples]
  }
