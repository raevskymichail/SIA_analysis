...
#' @import scales
#' @export
scale_three_colour_Publication <- function(...) {
  # Green, Red, Blue
  discrete_scale("colour", "Publication", manual_pal(values = c("#04bb3b", "#fb746c", "#619cff")))
}
...
#'
#' @param data a data frame where `rows` are samples and `cols` are gene names (or other features)
#' @param metadata a vector with size of `rows` of \code{data} containing an annotation per sample
#'
#' @import FactoMineR
#' @import factoextra
#' @import ggplot2
...
plot_pca <- function(data, metadata) {
  ...
  p <- fviz_pca_ind(data.pca,
                  ...
                  col.ind = metadata, # color by groups
                  addEllipses = FALSE, # Concentration ellipses
                  legend.title = NULL
                  ) +
                  labs(x = pc1_lab, y = pc2_lab) +
                  theme_Publication() +
                  scale_three_colour_Publication() +
