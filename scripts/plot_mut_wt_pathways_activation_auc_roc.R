...
# /usr/bin/Rscript ./scripts/main/plot_mut_wt_pathways_activation_auc_roc.R --gene="EGFR" \
#                              --pathways-activations-db="data/pathways_activation_scores/EGF_PWs.xlsx" \
#                              --output-dir="analyses/EGFR/any/lung_cancer" \
#                              --output-filename="EGFR_mut_wt_pathways_activation_auc_roc" \
...
# Calculate AUC ROC
plots_list <- calculate_roc_per_pw(sample_names = sample_pw_ann$sample_names,
                                     is_mutated = as.factor(sample_mut_ann[sample_pw_ann$sample_names]),
                                     pathways_names = gsub(" - ", "\n", sample_pw_ann$pathways_names),
                                     pw_activation_score = as.numeric(sample_pw_ann$pw_activation_score),
                                     gene = opt$gene,
                                     aa_change = opt$aa_change)

# do.call(grid.arrange, c(plots_list, ncol = plot_ncol))

export_analysis_plot(
  filename = opt$output_filename,
  plot = grid.arrange(plots_list[[1]], plots_list[[2]], ncol = 2),
  path = opt$output_dir,
  scale = 1,
  width = 7 * 2,
  height = 7,
